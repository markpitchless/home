" Only do this when not done yet for this buffer
if exists("b:did_ftplugin")
    finish
endif
  

"
" Maps                                                                    Maps
"=============================================================================

" TODO: Just write this on new file (use a skeleton)
map <> a<html><head><title></title></head><body><h1></h1></body></html>kkkkkkka

"
"Formating                                                           Formating
"=============================================================================

" Smaller tabs and wide screen as HTML is lardy!
setlocal shiftwidth=2
setlocal tabstop=2
setlocal textwidth=120

"
" Abbreviations                                                  Abbreviations
"=============================================================================

" Seperator comment.
ab <!---- <!-- ----------------------------------------------------------------------- -->
ab <!--== <!-- ======================================================================= -->


"
" Syntax options                                                Syntax options
"=============================================================================

" Turn of HTML highlights
"let html_no_rendering=1

" Allow user defined colouring (See docs or book for highlight groups)
"let html_my_rendering=1

" Highlight <!-- --!> and <! !> (illegal) comments
"let html_wrong_comments=1

"
" Pull in XML stuff

" Vim script file                                           vim600:fdm=marker:
" FileType:	HTML
" Maintainer:	Devin Weaver <vim@tritarget.com>
" Last Change:  $Date: 2002/11/19 23:06:35 $
" Version:      $Revision: 1.2 $
" Location:	http://tritarget.com/pub/vim/ftplugin/html.vim

" This is a wrapper script to add extra html support to xml documents.

" Only do this when not done yet for this buffer
if exists("b:did_ftplugin")
  finish
endif
" Don't set 'b:did_ftplugin = 1' because that is xml.vim's responsability.

if !exists("*HtmlAttribCallback")
function HtmlAttribCallback( xml_tag )
    if a:xml_tag ==? "table"
	return "cellpadding=\"0\" cellspacing=\"0\" border=\"0\""
    elseif a:xml_tag ==? "link"
	return "href=\"/site.css\" rel=\"StyleSheet\" type=\"text/css\""
    elseif a:xml_tag ==? "body"
	return "bgcolor=\"white\""
    elseif a:xml_tag ==? "frame"
	return "name=\"NAME\" src=\"/\" scrolling=\"auto\" noresize"
    elseif a:xml_tag ==? "frameset"
	return "rows=\"0,*\" cols=\"*,0\" border=\"0\""
    else
	return 0
    endif
endfunction
endif

" On to loading xml.vim
runtime ftplugin/xml.vim

